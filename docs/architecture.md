...
- **Единый первоисточник по протоколу.** Прошивка и хост используют одну
  ...
  Сейчас канал — текстовый CLI ([cli.md](cli.md)); бинарный протокол из
...
- Бинарный протокол (`shared/protocol/protocol.yaml`, `middleware/protocol/`) —
...
| `protocol.md`               | человекочитаемое описание бинарного протокола (первоисточник — YAML в `shared/`) |
...
### `shared/protocol/`
**Единый первоисточник по протоколу обмена.** Здесь определены все команды,
...
другой, бинарный канал, ещё не задействованный).
...
5. **Кодогенерируемые файлы помечаются комментарием «НЕ РЕДАКТИРОВАТЬ ВРУЧНУЮ»**
   ...
   как только бинарный канал будет подключён к диспетчеру.)
