...
- **Единый первоисточник по протоколу.** Прошивка и хост используют одну
  и ту же грамматику команд; рассинхронизация невозможна по построению.
  Сейчас канал — текстовый CLI ([cli.md](cli.md)); бинарный протокол
  запланирован, но пока не задействован и в репозитории не описан.
...
- Бинарный протокол (`firmware/src/middleware/protocol/`) — запланирован,
  **в сборку не подключён**, диспетчера кадров ещё нет.
...
Связь между `firmware/` и `software/` идёт через текстовый CLI поверх UART
(см. [cli.md](cli.md)).
...
| `protocol.md`               | человекочитаемое описание целевого бинарного протокола (запланирован, ещё не задействован) |
...
- `0003-protocol-binary.md` — формат протокола.
- `0004-pyqt6-not-pyqt5.md` — почему Qt6.

---

## 4. `firmware/` — прошивка STM32F407
...
### 4.5. `firmware/src/` — исходный код прошивки
...
| `protocol/` | заглушка под бинарный протокол: `protocol_parser.{h,c}` (структуры frame'а, статусы) и `protocol_gen.h` (артефакт кодогенерации). **В сборку не подключён** — диспетчер кадров ещё не написан, транспорт идёт через текстовый shell. |
...
`devices/{ah49e,ds18b20}`, `drivers/{uart,adc_probe}`. Первоисточник
по этому протоколу — [shell.h](../firmware/src/middleware/shell/inc/shell.h)
+ [docs/cli.md](cli.md). Запланированный бинарный канал (см. [protocol.md](protocol.md)) —
отдельная история, ещё не задействован.
...
## 5. `software/` — десктопное GUI

PySide6-приложение, общается с прошивкой по UART (текстовый CLI из
[cli.md](cli.md)). Архитектура — **классический MVC**, первоисточник
...
### 5.2. `software/controller_app/` — пакет приложения
...
### 5.3. Чего нет (по сравнению с «классической» структурой)
...
до бинарного канала — слои добавятся, и тогда обновить этот раздел.

---

## 6. `tools/` — вспомогательная автоматизация
...
## 7. `tests/integration/` — сквозные тесты
...
1. **Зависимости только сверху вниз.** Прикладной код прошивки не должен
   ...
   компилятором — см. §4.5 о `middleware/shell` как узаконенном исключении.)
   На хосте: views ничего не знают о serial-порте напрямую — только
   через controller.
2. **Один первоисточник.** Пины — `bsp_pins.h`. Версия прошивки —
   `bsp_version.h`. Текстовый shell-протокол — [shell.h](../firmware/src/middleware/shell/inc/shell.h)
...
| новую команду бинарного протокола (план)         | сначала описать формат кадра в [protocol.md](protocol.md), затем диспетчер во `firmware/src/middleware/protocol/` (когда будет написан) |
